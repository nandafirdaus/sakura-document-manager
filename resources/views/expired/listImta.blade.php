	...
	<div class="row">
		<div class="col-md-10 col-md-offset-1">
			<div class="panel panel-default">
				<div class="panel-heading">Daftar IMTA Expired</div>
...
<!--					<div class="form-group">
						<a class="btn btn-primary btn-sm" href="{{ url('/report/imta') }}" role="button">Download Laporan</a>
					</div>-->
					...
					<div class="table-responsive">
						<table class="table table-hover">
							...
							<tbody>
								@foreach ($imtas->all() as $imta)
									<tr>
										<td>{{$imta->kitas->employee->name}}</td>
										<td>{{$imta->kitas->employee->company->name}}</td>
										<td>{{$imta->doc_number}}</td>
										<td>{{date('d-m-Y', strtotime($imta->issued))}}</td>
										<td>{{date('d-m-Y', strtotime($imta->expired))}}</td>
										<td>
											<a class="btn btn-success" href="{{ url('/document/' . $imta->id . '/view?prev=expired') }}" role="button">Lihat</a>
											<a class="btn btn-primary" href="{{ url('/document/' . $imta->id . '/edit?prev=expired') }}" role="button">Edit</a>
											<a class="btn btn-warning" href="{{ url('/document/' . $imta->id . '/delete?prev=expired') }}" role="button" Onclick="javascript:return confirm('Yakin ingin menghapus data ini?');">Hapus</a>
										</td>
									</td>
								@endforeach
							</tbody>
